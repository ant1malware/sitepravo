...
function MemberAvatar({ member }: { member: Account }) {
  ...
  return (
    <div className="relative h-16 w-16 overflow-hidden rounded-2xl border border-white/10 shadow-[0_20px_45px_-25px_rgba(15,23,42,0.9)]">
      {member.profile.avatarData ? (
        <img src={member.profile.avatarData} alt={member.username} className="h-full w-full object-cover" loading="lazy" />
      ) : (
        <div className="flex h-full w-full items-center justify-center" style={{ background: `linear-gradient(135deg, ${accentFrom}, ${accentTo})` }}>
          <span className="text-lg font-semibold uppercase tracking-wide text-white/90">{initials}</span>
        </div>
      )}
    </div>
  );
      ...
      try {
        ...
        if (meRemote || rows?.length) {
          const adapted: Account[] = rows.map((u: PublicMember) => {
            ...
            const avatar =
              profile?.avatarData || (profile as any)?.avatar || (profile as any)?.avatarUrl || (profile as any)?.avatarURL || (profile as any)?.image || (profile as any)?.photo || (profile as any)?.picture || undefined;
            const banner =
              profile?.bannerData || (profile as any)?.banner || (profile as any)?.bannerUrl || (profile as any)?.bannerURL || undefined;
            ...
            const lastActiveAt = (u as any).lastActiveAt || (u as any).lastSeenAt || (u as any).lastOnlineAt || null;
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        {(() => {
          ...
          const online = visibleMembers.filter((m) => {
            ...
            return !Number.isNaN(t) && t > now - five;
          });
          if (!online.length) return null;
          return (
            <section className="mb-6 rounded-3xl border border-white/10 bg-white/[0.05] p-4 backdrop-blur">
              <div className="mb-3 text-xs uppercase tracking-[0.32em] text-emerald-200/80">Online now</div>
              <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {online.slice(0, 12).map((m) => (
                  <Link key={m.id} to={`/forum/profile/${m.username}`} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-3 py-2 hover:border-emerald-400/50">
                    <MemberAvatar member={m} />
                    <div className="min-w-0">
                      <div className="truncate font-semibold text-white">{m.username}</div>
                      <div className="text-[11px] uppercase tracking-[0.28em] text-zinc-300/70">{formatAgo((m as any).lastActiveAt)}</div>
                    </div>
                  </Link>
                ))}
              </div>
