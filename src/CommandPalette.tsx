...
export default function CommandPalette() {
  ...
  const baseRows: Row[] = React.useMemo(() => {
    const list: Row[] = [];
    list.push({ kind: 'page', id: 'forum', title: 'Ôîðóì', subtitle: 'Íàâèãàöèÿ', url: baseUrl('/forum') });
    list.push({ kind: 'page', id: 'favorites', title: 'Èçáðàííîå', subtitle: 'Ñòðàíèöà', url: baseUrl('/favorites') });
    list.push({ kind: 'page', id: 'settings', title: 'Íàñòðîéêè', subtitle: 'Ñòðàíèöà', url: baseUrl('/settings') });
    for (const r of rolesData) list.push({ kind: 'role', id: r.id, title: r.role, subtitle: 'Ðàçäåë', url: baseUrl(`/roles/${r.id}`) });
    for (const l of lawsData) list.push({ kind: 'law', id: l.slug, title: l.title, subtitle: 'Çàêîí', url: baseUrl(`/laws/${l.slug}`) });
    for (const v of vuDocs) list.push({ kind: 'vu', id: v.id, title: v.title, subtitle: 'ÂÓ', url: baseUrl(`/vu/${v.id}`) });
      ...
      for (const raw of lines) {
        ...
        if (m) {
          ...
          if (curId && headL.includes(qq)) {
            ...
            if (!added.has(key)) {
              const row: Row = { kind: 'law', id: key, title: `${l.title} — ${curHead}`, subtitle: 'Çàãîëîâîê', url: baseUrl(`/laws/${l.slug}?q=${encodeURIComponent(qq)}#${curId}`) };
  ...
  return (
    <>
      {!open ? null : (
        <div className="fixed inset-0 z-[70] flex items-start justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-label="Ïîèñê">
          <Panel>
            <div className="mb-2 flex items-center gap-2">
              ...
              <input
                ...
                onKeyDown={onKeyList}
                placeholder="Ïîèñê: çàêîí, ðîëü, ÂÓ..."
              ...
              />
              <button className="btn text-zinc-600 dark:text-zinc-300" onClick={() => setOpen(false)} aria-label="Çàêðûòü"><X className="h-4 w-4" /></button>
            </div>
            <div className="mb-2 flex items-center gap-2">
              {([
                {k:'all', label:'Âñå'},
            ...
            <div className="max-h-80 overflow-y-auto rounded-xl border border-zinc-200 dark:border-zinc-700">
              {!rows.length && (
                <div className="p-3 text-sm text-zinc-500 dark:text-zinc-400">Íè÷åãî íå íàéäåíî</div>
              )}
              {rows.map((r, i) => (
                <button
                  key={`${r.kind}:${r.id}`}
