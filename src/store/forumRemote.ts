...
export async function listSections(): Promise<Section[]> {
  const { sections } = await api('/sections'); return sections as Section[];
}
export async function createSection(input: { title: string; description?: string; icon?: string }): Promise<Section> {
  const { section } = await api('/sections', { method: 'POST', body: JSON.stringify(input) }); return section as Section;
}
...
export async function listTopics(sectionId?: string): Promise<Topic[]> {
  ...
  const { topics } = await api(`/topics${qs}`); return topics as Topic[];
}
export async function createTopic(input: { sectionId: string; title: string; content?: string }): Promise<Topic> {
  const { topic } = await api('/topics', { method: 'POST', body: JSON.stringify(input) }); return topic as Topic;
}
export async function updateTopic(id: string, patch: Partial<Topic>): Promise<Topic> {
  const { topic } = await api(`/topics/${id}`, { method: 'PATCH', body: JSON.stringify(patch) }); return topic as Topic;
}
