...
// English role names for UI (labels only)
const ROLE_META_EN: Record<string, { label: string; grad: string; Icon: any }> = {
  ...
  vip:       { label: "Premium",    grad: "from-amber-400 via-rose-400 to-amber-400",  Icon: Crown },
          ...
          <div className="relative -mt-16 flex flex-col gap-5 px-6 pb-6 sm:flex-row sm:flex-wrap sm:items-end">
            ...
            <div className="min-w-0 flex-1 space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-300/90">
                <span>Joined: {profileDateFormatter.format(new Date(user.createdAt))}</span>
              ...
              {(() => { const la = (user as any).lastActiveAt || getLastActive(user.id); return la ? (
                <div className="text-xs text-zinc-300/90">
                  Last seen: {formatLastActive(la)}
                </div>
              ) : null; })()}
