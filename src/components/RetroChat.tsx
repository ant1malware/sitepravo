...
export default function SimpleChat({
  ...
  React.useEffect(() => { if (isAdmin) { try { setNeedNick(false as any);} catch {} try { if (!nick) setNick('Admin' as any);} catch {} } }, [isAdmin]);
  ...
  const getRoleMeta = React.useCallback((name: string) => {
    ...
    if (name.trim().toLowerCase() === 'admin') return { label: 'Admin', color: '#f87171', textColor: '#fee2e2' };
    return null;
  }, [resolveRole]);
                ...
                <li key={m.id} className={`sb-li sb-has-avatar${mentionForMe ? ' sb-mention' : ''}${isSystem ? ' sb-system' : ''}`}>
                  ...
                  <div className="sb-row">
                    <span className={`sb-name ${nickClass(m.author)}`}>
                      {role ? <>
                        <span className="sb-role">({role.label})</span>&nbsp;
                      </> : null}
                      <Link to={`/forum/profile/${encodeURIComponent(m.author)}`}>{m.author}</Link>
                    </span>
