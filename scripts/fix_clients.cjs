...
// Patch authRemote.ts (insert missing constants and types if absent)
{
  const file = 'src/store/authRemote.ts';
  let s = fs.readFileSync(file, 'utf8');
  if (!s.includes('const TOKEN_KEY')) {
    s = s.replace(
  ...
  console.log('authRemote.ts fixed');
}

// Rewrite forumRemote.ts to a consistent version with getApiBase
{
  const file = 'src/store/forumRemote.ts';
  const content = String.raw`// Remote forum store (Cloudflare Worker API)
...
export async function listTopics(sectionId?: string): Promise<Topic[]> { const qs = sectionId ? `?sectionId=${encodeURIComponent(sectionId)}` : ''; const { topics } = await api(`/topics${qs}`); return topics as Topic[]; }
export async function createTopic(input: { sectionId: string; title: string; content?: string }): Promise<Topic> { const { topic } = await api('/topics', { method: 'POST', body: JSON.stringify(input) }); return topic as Topic; }
export async function updateTopic(id: string, patch: Partial<Topic>): Promise<Topic> { const { topic } = await api(`/topics/${id}`, { method: 'PATCH', body: JSON.stringify(patch) }); return topic as Topic; }
export async function moveTopic(id: string, to: string): Promise<Topic> { const { topic } = await api(`/topics/${id}/move`, { method: 'POST', body: JSON.stringify({ to }) }); return topic as Topic; }
...
export async function listLatestPosts(limit = 8): Promise<Post[]> { const { posts } = await api(`/latest-posts?limit=${limit}`); return posts as Post[]; }
`;
  fs.writeFileSync(file, content, 'utf8');
  console.log('forumRemote.ts rewritten');
}
